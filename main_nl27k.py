...
def set_parser():
    ...
    # required
    parser.add_argument('--data', type=str, default='nl27k', help="the dir path where you store data (train.tsv, val.tsv, test.tsv). Default: ppi5k")
    # optional
    ...
    parser.add_argument('--bert_model', type=str, default='bert-base-uncased',help="BERT模型名称，默认bert-base-uncased")
    parser.add_argument('--freeze_bert', type=bool, default=True,help="是否冻结BERT参数。True=不微调BERT, False=微调BERT")
    parser.add_argument('--regenerate_bert', type=bool, default=False,help="是否重新生成BERT embeddings（即使缓存存在）。默认False，会优先加载缓存")
    ...
    parser.add_argument('--min_weight', type=float, default=0.85,help="邻居三元组的最小权重阈值。默认0.85")

    # 聚类相关参数
    parser.add_argument('--use_clustering', type=bool, default=True,help="是否使用聚类增强模块（簇共享embedding+门控融合+对比学习）")
    parser.add_argument('--num_entity_clusters', type=int, default=130,help="实体簇数量。推荐: CN15K=70, NL27K=130, PPI5K=50")
    parser.add_argument('--num_relation_clusters', type=int, default=30,help="关系簇数量。推荐: CN15K=10, NL27K=30, PPI5K=3")
    parser.add_argument('--regenerate_clustering', type=bool, default=False,help="是否重新执行聚类（即使缓存存在）。默认False，会优先加载缓存")
